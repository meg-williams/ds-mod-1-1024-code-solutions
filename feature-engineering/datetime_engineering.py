...
def create_month(df: pd.DataFrame, feature: str, name: str) -> pd.DataFrame:
    ...
    # TODO: Implement this function
    df[name] = pd.to_datetime(df[feature]).dt.month
    return df
...
def aggregate_months(df: pd.DataFrame, group: str, agg_func: str) -> pd.DataFrame:
    ...
    # TODO: Implement this function
    if agg_func == 'np.mean':
        df_agg = df.groupby(group).np.mean()
        return df_agg
    elif agg_func == 'np.sum':
        df_agg = df.groupby(group).np.sum()
        return df_agg
    elif agg_func == 'np.max': 
        df_agg = df.groupby(group).np.max()
        return df_agg
    elif agg_func == 'np.min': 
        df_agg = df.groupby(group).np.min()
        return df_agg
