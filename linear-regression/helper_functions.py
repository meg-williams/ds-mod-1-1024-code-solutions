...
def create_equatorial_distance_feature(df) -> pd.DataFrame:
    ...
    df['distance_from_equator_bin'] = pd.cut(df['distance_from_equator'], 10, labels=False)
...
def linear_regression_pipeline(
    ...
    lr_pipline = LinearRegression()
    lr_pipline.fit(X_train, y_train)

    # Make predictions on the X_train and X_test datasets
    y_pred_train = lr_pipline.predict(X_train)
    y_pred_test = lr_pipline.predict(X_test)

    # inverse transform all 4 target variables (y_train, y_pred_train, y_test, y_pred_test)
    if target_transformer:
        y_train = target_transformer.inverse_transform(y_train.reshape(-1, 1)).flatten()
        y_pred_train = target_transformer.inverse_transform(y_pred_train.reshape(-1, 1)).flatten()
        y_test = target_transformer.inverse_transform(y_test.reshape(-1, 1)).flatten()
        y_pred_test = target_transformer.inverse_transform(y_pred_test.reshape(-1, 1)).flatten()
    ...
    return lr_pipline, train_metrics, test_metrics
...
def scale_features(X_train, X_test, feature_cols) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    X_train[feature_cols] = rs.fit_transform(X_train[feature_cols])
...
def transform_target(
    ...
    y_test = transformer.transform(y_test.values.reshape(-1,1)).flatten()
...
def backward_stepwise_train(X_train, y_train, feature_cols):
    ...
    yval_preds = model.predict(xval[remaining_features])
    yval_preds_trans = target_transformer.inverse_transform(yval_preds.reshape(-1,1))
    yval_trans = target_transformer.inverse_transform(yval.reshape(-1,1))
    rmse = mean_squared_error(yval_trans, yval_preds_trans)**0.5
    ...
    for i in range(1, k):
        ...
        for feat in remaining_features:
            ...
            yval_preds = model.predict(xval[features])
            yval_preds_trans = target_transformer.inverse_transform(yval_preds.reshape(-1,1))
            yval_trans = target_transformer.inverse_transform(yval.reshape(-1,1))
            rmse = mean_squared_error(yval_trans, yval_preds_trans)**0.5
...
def build_lasso_model(X_train, y_train, alpha):
    ...
    sse = mean_squared_error(yval, preds)*p
    rmse = np.sqrt(sse/n)
